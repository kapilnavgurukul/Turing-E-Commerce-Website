module.exports=(shoppingcart,knex,jwt,secret_key)=>{
    ...
    shoppingcart.post('/add',(req,res)=>{
        ...
        .where({'cart_id':customer_id,'product_id':product_id,'attributes':attributes})
        .then((data)=>{
            if (data.length>0){
    ...
    shoppingcart.get("/",(req,res)=>{
        ...
        let customer_id=parseInt(jwt.verify(token,secret_key).customer_id)
        knex('product')
        .select('product.product_id','name','image','price','item_id','quantity')
        .join('shopping_cart','product.product_id','shopping_cart.product_id')
        .where({
            'shopping_cart.cart_id':customer_id,
            "buy_now":1
            })
        .then((data)=>{
    ...
    shoppingcart.put("/update",(req,res)=>{
        ...
        knex('shopping_cart')
        .where({'product_id':item_id,'cart_id':cart_id})
    ...
    shoppingcart.get("/totalAmount",(req,res)=>{
        ...
        let cart_id=parseInt(jwt.verify(token,secret_key).customer_id)
        knex('product')
        .select('product.product_id','price','quantity')
        .join('shopping_cart','product.product_id','shopping_cart.product_id')
    ...
    shoppingcart.delete("/removeproduct",(req,res)=>{
        ...
        knex("shopping_cart")
        .where({'shopping_cart.cart_id':cart_id,'item_id':item_id})
        .del('*')
        .then(()=>{
            console.log("yes") 
        ...
        .then((data)=>{
            ...
            .then((data1)=>{
                if (data1.length>0){
                    ...
                    .del('*')
                    .then(()=>{res.send("save for letter successful")})
                    .catch((err)=>{res.send(err)})}
                else{
                    knex('save_cart')
                    .insert({
    ...
    shoppingcart.get('/movetocart',(req,res)=>{
        ...
        .then((data)=>{
            ...
            .then(()=>{
                knex('save_cart')
                    .where({'item_id':item_id,'cart_id':cart_id})
                    .del('*').then(()=>{res.send("successful add to cart")})
                    .catch((err)=>{res.send(err)})
                })
        })
        .catch((err)=>{res.send(err)})
    ...
    shoppingcart.get("/getsaved",(req,res)=>{
        ...
        knex('save_cart')
        .select('product.name','save_cart.item_id','save_cart.attributes','product.price')
