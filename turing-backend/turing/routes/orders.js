module.exports=(orders,knex,jwt)=>{
    orders.post("/",(req,res)=>{
        ...
        let date =new Date
        console.log(date)
        knex('shopping_cart').select('*').where('cart_id',cart_id)
        ...
        .then((data)=>{
            ...
            .then((ship_data)=>{
                ...
                .then((tax_data)=>{
                    ...
                    total_amount+=tax
                    knex('orders').insert({'total_amount':total_amount,'created_on':date,'shipping_id':ship_id,'tax_id':tax_id,'status':1,'customer_id':cart_id})
                    .then(()=>{
